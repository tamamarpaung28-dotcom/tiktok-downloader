    ...
    async with httpx.AsyncClient(follow_redirects=True, timeout=20.0) as client:
        try:
            ...
            if data.get("code") == 0:
                ...
                if not download_link:
                    raise HTTPException(status_code=400, detail="Format media tidak ditemukan.")
                ...
                raise HTTPException(status_code=400, detail="Gagal mengambil video TikTok.")
        except Exception as e:
            raise HTTPException(status_code=400, detail=f"Terjadi kesalahan: {str(e)}")

# 🛑 PENTING: BACA PORT DARI RAILWAY KARENA DYNAMIC
if __name__ == "__main__":
    port = int(os.environ.get("PORT", 8000))
    uvicorn.run(app, host="0.0.0.0", port=port)
